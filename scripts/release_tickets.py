...
the pull request with the state of every surface. The release has a
milestone, "<capability> <version>": the tool creates it when missing
and sets it on every ticket and on the pull request, so the release's
issues are one list; it never closes the milestone (a milestone closes
when its issues are done, the maintainer's call). Idempotent: a re-run
updates rather than duplicates. It needs the workflow's own token
(issues: write, which covers milestones, and pull-requests: write) and
nothing secret: the runs themselves happen on the maintainers' machines
and accounts, the way users would run them.
...
def milestone_title(cap: dict) -> str:
    return f"{cap['name']} {cap['version']}"


def ensure_milestone(repo: str, cap: dict, dry_run: bool) -> int | None:
    """The number of the release milestone, created when missing; None on a
    dry run that would create it."""
    title = milestone_title(cap)
    for m in gh([f"repos/{repo}/milestones?state=all&per_page=100"]) or []:
        if m.get("title") == title:
            return m["number"]
    created = gh(["-X", "POST", f"repos/{repo}/milestones"], dry_run,
                 {"title": title, "description": f"The {cap['name']} {cap['version']} release: its qualification tickets and release pull request."})
    if created:
        print(f"created milestone {created['number']}: {title}")
...
def milestone_number(issue: dict) -> int | None:
    ...
    return m.get("number") if isinstance(m, dict) else None
...
def sync(repo: str, pr: int, cap: dict, dry_run: bool) -> int:
    ...
    milestone = ensure_milestone(repo, cap, dry_run)
    ...
    for runtime, decision in state.items():
        ...
        if decision == "missing":
            ...
            if issue is None:
                payload = {"title": title, "body": body, "labels": [LABEL, f"runtime:{runtime}"]}
                if milestone is not None:
                    payload["milestone"] = milestone
                created = gh(["-X", "POST", f"repos/{repo}/issues"], dry_run, payload)
                ...
                if (issue.get("body") or "").strip() != body.strip():
                    patch["body"] = body
                if milestone is not None and milestone_number(issue) != milestone:
                    patch["milestone"] = milestone
                if patch:
                    gh(["-X", "PATCH", f"repos/{repo}/issues/{issue['number']}"], dry_run, patch)
    ...
    summary = (f"## Release candidate {name} {version}: runtime qualification\n\n" + "\n".join(lines) + "\n\n"
               ...
               + "\n---\n_Generated by [Claude Code](https://claude.ai/code)_")
    if milestone is not None:
        pull = gh([f"repos/{repo}/issues/{pr}"]) or {}
        if milestone_number(pull) != milestone:
            gh(["-X", "PATCH", f"repos/{repo}/issues/{pr}"], dry_run, {"milestone": milestone})
            print(f"milestone {milestone_title(cap)} set on pull request #{pr}")
